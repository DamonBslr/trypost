...
- The client wiring is still shipped (`resources/js/ssr.ts`, `vite.config.ts`, `npm run build:ssr` for local use). Production images run `npm run build` only. Turning SSR on means building that bundle and running `inertia:start-ssr` alongside the app, not just flipping the env.
